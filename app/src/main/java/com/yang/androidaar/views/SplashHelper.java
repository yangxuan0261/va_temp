...
public class SplashHelper implements SplashInter {
    static SplashHelper instance = null;
    ...
    public void createView() {
        ...
        try {
            // 混淆名字
            String splashValue = Tools.GetStringVaule(ActivityMgr.getIns().getActivity(), Define.getResKey_Splash());
            String splashName = splashValue != null ? splashValue : Define.getdDefault_Splash();
            
            splashView = addFullScreenImage(splashName);
            // 开始显示闪屏
    ...
    public void hideSplash() {
        ...
        Runnable closeSplashFn = new Runnable() {
            @Override
            public void run() {
                LogUtil.TD(TAG, LogUtil.PGFmt("--- run: closeSplashFn"));
                mActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            ...
                            Log.e(TAG, LogUtil.PGFmt("--- run: closeSplashFn, err:") + e.getMessage());
                        }
                    }
                });
            }
